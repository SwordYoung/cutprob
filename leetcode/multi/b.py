...
class Solution:
    ...
    def add(self, sum, add):
        ...
        c = 0
        for i in range(l):
            add_l = 0 if i >= len(sum) else int(sum[i])
            add_r = 0 if i >= len(add) else int(add[i])
    ...
    def multiply(self, num1, num2):
        ...
        for i in range(len(n2)):
            newadd = self.mul(n1, n2[i], i)
            adds.append(newadd)
        sum = [0]
        for i in range(len(adds)):
            sum = self.add(sum, adds[i])
        stt = len(sum)-1
        while sum[stt] == 0 and stt > 1:
            stt -= 1
        sum = sum[:stt]
        sum.reverse()
